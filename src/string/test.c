...
void floatToString(float num, char *str, int precision) {
  ...
  if (precision > 0) {
    ...
    // Умножаем дробную часть на 10^precision, чтобы получить нужное количество
    // знаков после запятой
    for (int i = 0; i < precision; i++) {
      fractionalPart *= 10;
      int digit = (int)fractionalPart; // Получаем очередную цифру после запятой
      str[len++] =
          '0' + digit; // Преобразуем цифру в символ и добавляем в строку
      fractionalPart -=
          digit; // Исключаем уже добавленную цифру из дробной части
    }
  }
